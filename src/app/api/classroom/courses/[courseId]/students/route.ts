...
export async function GET(req: NextRequest, { params }: { params: { courseId: string } }) {
  ...
  try {
    const res = await fetch(`https://classroom.googleapis.com/v1/courses/${courseId}/students?${qs.toString()}`, {
    ...
    if (!res.ok) return NextResponse.json({ error: 'Error al obtener estudiantes', status: res.status, details: text }, { status: res.status })
